...
const saveLikedArtworkIntoDataBase = async function (artwork ) {
    ...
    const response = await fetch(mainURL + `saveArtworkIntoDataBase`, requestOptions).then(function (response) {
        if (!response.ok) {
...
export default saveLikedArtworkIntoDataBase;
